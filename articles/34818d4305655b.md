...
4. **AutomationAssumeRole の未指定**：Systems Manager Automationが正しく実行されなかった可能性
...
```json:IAMロールポリシー
{
  ...
  "Statement": [
    ...
    // SSM の Automation機能 に関するアクセス許可を定義
    {
    ...
    // AutomationAssumeRole の指定
    {
      "Sid": "AllowPassRoleForAutomation",
      "Effect": "Allow",
      "Action": "iam:PassRole",
      "Resource": "arn:aws:iam::アカウント番号:role/実行IAMロール名"
    }
  ]
}
```
